...
func (h *Order) Create(w http.ResponseWriter, r *http.Request) {
	...
	order := model.Order{
		OrderID:    rand.Uint64(), // dangerous for production. just for testing
...
func (h *Order) List(w http.ResponseWriter, r *http.Request) {
	...
	var response struct {
		...
		Next  uint64        `json:"next,omitempty` // omitempty -> if nil give default value for this 0
	}
...
func (h *Order) GetById(w http.ResponseWriter, r *http.Request) {
	idParam := chi.URLParam(r, "id") // catch id in url

	const base = 10
	const bitSize = 64
	orderID, err := strconv.ParseUint(idParam, base, bitSize) // convert string to 64 byte uint
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		return
	}
